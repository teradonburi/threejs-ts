...
export default class Game {
  ...
  objectTimePeriod: number = 0
  timeNextSpawn: number = 0
  prevPressC: boolean = false
  jumbTime: number = 0
  ...
  init = async () => {
    ...
    this.controlCamera.controls = this.controlCamera.createControls(this.renderer)
    ...
    // Sky
    this.sky = new Sky()
    this.sky.setEnv({turbidity: 10, rayleigh: 2, mieCoefficient: 0.005, mieDirectionalG: 0.8})
    ...
    for (let i = 0; i < 5; i++) {
      ...
      this.physicsWorld.addBoxBody(this.trees[i].object, this.trees[i].object.size!.multiplyScalar(0.8), 0, true)
    ...
    this.physicsWorld.addHumanBody(this.model.object, this.model.object.size, 0.5)
  ...
  onDragStart = (e: { object: THREE.Mesh & { userData?: { ignorePhysics: boolean } } }) => {
  ...
  onDragEnd = (e: { object: THREE.Mesh & { userData?: { ignorePhysics: boolean } } }) => {
    if (e.object.userData) {
  ...
  render = (frame: number) => {
    ...
    // 物理空間上のオブジェクトの当たり判定
    const hitResult = this.physicsWorld.hitTest([this.heightMap, this.model.object])
