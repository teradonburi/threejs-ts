...
export default class HeightMap extends THREE.Mesh {
  ...
  constructor (size = 512, geometrySize = 512, minHeight = -1, maxHeight = 30) {
    ...
    const terrainMesh = this.createGeometry(size, size, geometrySize, geometrySize, heightData)
  ...
  generateHeight = (terrainWidth: number, terrainDepth: number, minHeight: number, maxHeight: number) => {
  ...
  // Mesh作成
  createGeometry = (terrainWidth: number, terrainDepth: number, terrainGeometoryWidth: number, terrainGeometoryHeight: number, heightData: Float32Array) => {
    const geometry = new THREE.PlaneGeometry(terrainGeometoryWidth, terrainGeometoryHeight, terrainWidth - 1, terrainDepth - 1)
  ...
  generateTexture = (data: Float32Array, width: number, height: number) => {
    ...
    if(context) {
      ...
      let image = context?.getImageData(0, 0, canvasScaled.width, canvasScaled.height)
      let imageData = image.data
