...
export default class PhysicsWorld {
  ...
  addSphereBody = (objThree: THREE.Object3D, radius: number, mass: number, isKinematic = false, isStatic = false) => {
  ...
  addBoxBody = (objThree: THREE.Object3D, size: THREE.Vector3, mass: number, isKinematic = false, isStatic = false) => {
  ...
  addCylinderBody = (objThree: THREE.Object3D, radius: number, height: number, mass: number, isKinematic = false, isStatic = false) => {
  ...
  addConeBody = (objThree: THREE.Object3D, radius: number, height: number, mass: number, isKinematic = false, isStatic = false) => {
  ...
  addCapsuleBody = (objThree: THREE.Object3D, radius: number, height: number, mass: number, isKinematic = false, isStatic = false) => {
  ...
  addConvexBody = (objThree: THREE.Object3D, vertices: THREE.Vector3[], mass: number, isKinematic = false, isStatic = false) => {
  ...
  // addTriangleBody = (objThree: THREE.Object3D, vertices: THREE.Vector3[], faces, mass: number, isKinematic = false, isStatic = false) => {
  ...
  addHumanBody = (objThree: THREE.Object3D, size = new THREE.Vector3(1,1,1), scale = 1, friction = 1, mass = 200) => {
    this.addCapsuleBody(objThree, (size.x ** 2 + size.z ** 2) ** 0.5 * 0.5 * scale, size.y, mass)
    // 回転させない
  ...
  // 物理世界の衝突オブジェクトを作成
  createBody = (mass: number, transform: Ammo.btTransform, shape: Ammo.btCollisionShape, isKinematic = false, isStatic = false) => {
  ...
  addImpulse = (objThree: THREE.Object3D, impulse = new THREE.Vector3(0, 0, 0)) => {
  ...
  // 物理世界の姿勢をモデルの描画姿勢に反映
  setModelPose = (objThree: THREE.Object3D & { center?: THREE.Vector3, boxHelper?: THREE.BoxHelper }) => {
  ...
  getPointer = (body: unknown) => (Ammo as any).getPointer(body)

  // 物理空間上のオブジェクトの当たり判定
  hitTest = (targets: THREE.Object3D[], needHitPoint = false) => {

    const targetPtrs: {[key: string]: Ammo.btCollisionObject | {}} = {}
    for (let key in targets) {
      const target = targets[key]
      if (!target.userData.physicsBody) continue
      targetPtrs[this.getPointer(target.userData.physicsBody)] = {}
    }
