...
export default class Sprite extends THREE.Sprite {
  private initWindowWidth: number = 0
  private initWindowHeight: number = 0
  private w: number = 0
  private h: number = 0
  ...
  setCenter = (align: {left?: boolean, top?: boolean, bottom?: boolean, right?: boolean, center?: boolean} = {left: false, top: false, bottom: false, right: false, center: true}) => {
    if (align.center) {
      this.center.set(0.5, 0.5)
  ...
  setSize = (w: number, h: number) => {
    this.w = w
    this.h = h
    const scale = (window.innerWidth + window.innerHeight) / (this.initWindowWidth + this.initWindowHeight)
