...
export default class Sky extends ThreeSky {
  ...
  setEnv = (params: {turbidity?: number, rayleigh?: number, mieCoefficient?: number, mieDirectionalG?: number}) => {
    const uniforms = this.material.uniforms
    if (params.turbidity) uniforms.turbidity.value = params.turbidity
    if (params.rayleigh) uniforms.rayleigh.value = params.rayleigh

    if (params.mieCoefficient) uniforms.mieCoefficient.value = params.mieCoefficient
    if (params.mieDirectionalG) uniforms.mieDirectionalG.value = params.mieDirectionalG
  }
